...
const Navbar = ({ onDivisionChange, onCategoryChange, onSearchChange }) => {
	...
	const [helloText, setHelloText] = useState('Welcome to the Nike Store');

	useEffect(() => {
		const interval = setInterval(() => {
			setHelloText((prevText) =>
				prevText === 'Welcome to the Nike Store' ? 'Unlock the world of luxury!' : 'Welcome to the Nike Store'
			);
		}, 2000);
